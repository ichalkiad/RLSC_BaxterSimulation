...
int main(int argc, char* argv[])
{
	...
	if (clientID!=-1)
	{
		...
		while (simxGetConnectionId(clientID)!=-1)
		{
			...
			// Stream motor angles:
			simxAppendStringSignal(clientID,motorPosSignalName.c_str(),(simxChar*)embeddedModelMotorPositions,3*4,simx_opmode_oneshot);
			// Check if the other side is reading the buffer:
			cnt++;
			if (cnt>2000)
			{
				if (simxGetIntegerSignal(clientID,acknSignalName.c_str(),&dummyInt,simx_opmode_buffer)!=simx_error_noerror)
